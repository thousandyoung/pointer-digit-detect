...
from sympy import *
import math
from getDigitPosition import get_0_1_position,cv2_to_base64
...
def needle(path, img, r, cx, cy, x0, y0):
    ...
    lines = cv2.HoughLinesP(mask, 1, np.pi / 180, 100,
                            minLineLength=int(r / 2), maxLineGap=2)
...
def GetAngle(path, outpath, opoint):
    ...
    kb, r, mask = linecontours(ninfo, path)
...
def GetpointerResult(inputPath):
    ...
    # 输出文件夹
    outputpath = 'output'
    data = {'images': [cv2_to_base64(cv2.imread(inputpath))]}
    headers = {"Content-type": "application/json"}
    url = "http://127.0.0.1:8866/predict/chinese_ocr_db_crnn_mobile"
    r = requests.post(url=url, headers=headers, data=json.dumps(data))

    p0 = list(get_0_1_position(r)[0])
    p1 = list(get_0_1_position(r)[1])
    p1_text = int(getOcrResult(r)[1]['text'])

    ang0 = GetAngle(inputpath, outputpath, p0)
    ang1 = GetAngle(inputpath, outputpath, p1)
    ang_between_0_1 = ang0 - ang1
    # print("ang0 {}, ang1 {}, ang_0_1 {}".format(ang0, ang1, ang_between_0_1))
    degree = p1_text * ang0 / ang_between_0_1
    print(degree)
    return degree

if __name__ == "__main__":
    GetpointerResult('5.jpg')
